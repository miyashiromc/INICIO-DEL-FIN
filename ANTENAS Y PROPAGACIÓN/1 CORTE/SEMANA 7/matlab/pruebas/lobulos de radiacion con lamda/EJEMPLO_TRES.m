% Dipole L = 3*lambda/2 - Patrón 3D y cortes H-plane y E-plane
clc; clear; close all;
...
c      = 3e8;
lambda = c/f;
L      = 3*lambda/2;  % dipolo de longitud 3*lambda/2
k      = 2*pi/lambda;
...
title('H-plane (phi=pi/2) - L=3*lambda/2');
...
title('E-plane (theta=0) - L=3*lambda/2');
